...
(function ( common, $ ) {
    ...
    $.fn.vinField = function () {
        var element = $(this);
        element.prop('maxLength', 17).attr("pattern", "[A-Z0-9]").attr("autocomplete", "off").attr("autocapitalize", "words");
        return this;
    };
    ...
    $.fn.noSpecialCharacters = function ( event ) {
        if ((event.which > 30 && event.which < 48) || (event.which > 57 && event.which < 65) || (event.which > 90 && event.which < 97) || (event.which > 122 && event.which < 127)) {
     ...
     */
    common.apiCall = function ( url, method, data, successCallBack, errorCallBack, async ) {
        var ajaxOptions = {
            type : method,
            url : url,
            dataType : "JSON",
        ...
        return $.ajax(ajaxOptions).done(function ( response ) {
            if (typeof successCallBack !== "undefined" && successCallBack !== null) {
                var data = perpetulist.cyclicJsonHandler.processJson(response);
        ...
        $(document).ready(function () {
            $(document).on("click", ".message .message-close", function () {
                ...
                if (msgContainer.hasClass("error")) {
                    msgContainer.fadeOut("slow", function () {
                        ...
                        msgContainer.find(".message-content").text("");
                    });
            ...
            $(document).on("click", ".fa-location-arrow", function () {
                if (!$(".address-field").prop('readonly')) {
                    ...
                    common.getCurrentLocation($addressField, $streetNumField, $streetNameField, $stateField);
                }
            });
            ...
            $(document).on("keydown", "input[type=email]", function ( event ) {
                var key = event.key;
                var re = /[a-zA-Z0-9@\.\_\-\+]/;
                return re.test(key);
            });
    ...
    common.setSidebarHeight = function ( sidebarElement, extraHeight ) {
        if (sidebarElement.length > 0) {
            var sidebarHeight = (typeof extraHeight !== "undefined") ? (sidebarHeight - extraHeight) : sidebarHeight;
            var scrollableElement = sidebarElement.find("ul");
            scrollableElement.css("height", sidebarHeight + "px").css("overflowY", "auto");
    ...
    common.showMessage = function ( msgContainer, msg, type ) {
        if ($(msgContainer).find(".message").length > 0) {
            ...
            msgContent.text(msg);
            msgContainerObj.addClass(type).fadeIn('slow').delay("4000").fadeOut("slow", function () {
                msgContent.text("");
    ...
    common.resizeImages = function ( element, imageUrl, containerWidth, containerHeight, successCallBack, failCallBack ) {
        var data = {
            imageUrl : imageUrl,
            width : parseInt(containerWidth),
            height : parseInt(containerHeight)
        };
        var response = common.getBaseUrl() + "unsafe/full-fit-in/" + data.width + "x" + data.height + "/filters:quality(" + common.getDynamicQuality(perpetulist.bandwidthDetectionHelper.getBandwidth()) + ")/" + data.imageUrl;
        var img = new Image();
        img.src = response;
     ...
     *        jQuery object of the field where street number is to be populated
     * @param streetNameElement
     *        jQuery object of the field where street name is to be populated
     * @author Niraj Rajbhandari <nirajrajbhandari@lftechnology.com>
     * @author Sanish Maharjan <sanishmaharjan@lftechnology.com>
     */
    common.enableGoogleSearch = function ( $addressField, $stateFieldElement, $streetNumFieldElement, $streetNameFieldElement ) {
        if (typeof google !== "undefined") {
            ...
            google.maps.event.addListener(autocomplete, 'place_changed', function () {
                ...
                if (place.geometry) {
                    if (typeof $stateFieldElement === "object" && typeof $streetNumFieldElement === "object" && typeof $streetNameFieldElement === "object") {
                        ...
                        for (var i = 0; i < place.address_components.length; i++) {
                            if ($.inArray("street_number", place.address_components[i].types) !== -1) {
                                $streetNumFieldElement.val(place.address_components[i].long_name);
                            } else if ($.inArray("route", place.address_components[i].types) !== -1) {
                                $streetNameFieldElement.val(place.address_components[i].long_name);
                            } else if ($.inArray("administrative_area_level_1", place.address_components[i].types) !== -1) {
                                $stateFieldElement.val(place.address_components[i].long_name);
                            }
                        }
                    }
                }
    ...
    common.getCurrentLocation = function ( $addressField, $stateFieldElement, $streetNumFieldElement, $streetNameFieldElement ) {
                        ...
                        if (response.results.length > 0) {
                            ...
                            for (var i = 0; i < result.address_components.length; i++) {
                                if ($.inArray("street_number", result.address_components[i].types) !== -1) {
                                    $streetNumFieldElement.val(result.address_components[i].long_name);
                                } else if ($.inArray("route", result.address_components[i].types) !== -1) {
                                    $streetNameFieldElement.val(result.address_components[i].long_name);
                                } else if ($.inArray("administrative_area_level_1", result.address_components[i].types) !== -1) {
                                    $stateFieldElement.val(result.address_components[i].long_name);
                                }
                            }
                            if (!$addressField.prop('readonly')) {
                                $(".fa-location-arrow").show();
    ...
    common.enableWYSIWYG = function ( elementSelector ) {
        var editor = new MediumEditor(elementSelector, {
            ...
            toolbar : {
                ...
                buttons : [ 'bold', 'italic', 'underline', 'anchor', 'h2', 'h3', 'quote', 'strikethrough', 'superscript', 'subscript', 'orderedlist', 'unorderedlist', 'removeFormat' ],
                diffLeft : 0,
                diffTop : -10,
